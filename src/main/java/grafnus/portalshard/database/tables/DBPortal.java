...
public class DBPortal extends DataTable {
    public DBPortal() {
        super("Portal", "CREATE TABLE IF NOT EXISTS `portal` (" +
    ...
    public static ArrayList<PortalData> getPortalByConnID(float cID) {
        ...
        String query = "SELECT `world`, `x`, `y`, `z`, `connection_id` FROM `portal` WHERE `connection_id` = ?;";
    ...
    public static float getPortalID(PortalData data) {
        String queryPortalID = "SELECT `id` FROM `portal` WHERE `connection_id` = ? AND `world` = ? AND `x` = ? AND `y` = ? AND `z` = ?;";
    ...
    public static float getConnectionID(PortalData data) {
        String queryPortalID = "SELECT `connection_id` FROM `portal` WHERE `world` = ? AND `x` = ? AND `y` = ? AND `z` = ?;";
        float[] id = new float[1];
        DataSource.getInstance().execute( (conn -> {
            PreparedStatement p = conn.prepareStatement(queryPortalID);
